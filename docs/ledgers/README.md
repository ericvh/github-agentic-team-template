...
GitLab is authoritative for live workflow state. These records preserve domain
evidence that cannot be reconstructed reliably from issues, merge requests, or
the code alone.
...
Do not duplicate assignees, issue state, MR state, or labels without an explicit
